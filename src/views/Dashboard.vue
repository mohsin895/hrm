                  ...
                  <table class="w-full min-w-max mb-2">
                    <tbody>
                      <tr v-for="(data,index) in birthdays" :key="index" class="bg-dashboardbg p-4">
                        <td class="flex border-b border-slate-300 pl-4 text-sm h-10">
                          <span> <icon name="user" size="15px" style="margin-right: 8px" /></span> <span class="font-bold">{{data.full_name}}</span> <span>has birthday on</span><span class="font-bold ml-1">{{ $filters.formatDateWithMonth(data.date_of_birth) }}</span>
                        </td>
                      ...
                      <tr v-for="(data,index) in awards.data" :key="index" class="bg-dashboardbg p-4">
                        <td class="border-b border-slate-300 pl-4 text-sm h-10">
                          <div class="flex justify-between">
                            ...
                            <div class="flex">
                              <span> <icon name="user" size="15px" style="margin-right: 8px" /></span> <span class="font-bold">{{ data.employee?.full_name || 'Unknown Employee' }}</span> <span>{{data.award_name}}</span><span class="font-bold ml-1">{{ $filters.formatDateWithMonth(data.date_of_birth) }}</span>
                            </div>
                            <div>
