<template>
  ...
  <table class="border-collapse border border-slate-400 w-full min-w-max">
    <thead>
      <tr>
        ...
        <th class="border border-slate-300">Leave</th>
        <th class="border border-slate-300">Number of Leaves</th>
        ...
        <div class="bg-white shadow-lg w-full max-w-lg" @click.stop>
          ...
          >
            <h2 class="text-xl inline-flex items-center font-semibold">
              ...
              /></span>
              {{ editMode ? "Edit leaves type" : "Add Leave Type " }}
            </h2>
            <button
              @click="closeModal"
            ...
            <div class="pl-6 pr-6 modal-body">
              <div class="mb-4">
                <label
                  ...
                  for="itemName"
                >
                  <span :class="{ 'text-red-500': nameError }">Type of Leave</span>
                </label>
                <input
                  v-model="leaveType"
                  type="text"
                  ...
                  @keyup="formValidation('leaveType')"
                />
              ...
              <div class="mb-4">
                <label
                  ...
                  for="giftName"
                >
                  <span :class="{ 'text-red-500': nameError }">Number of Days </span>
                </label>
                <input
                  v-model="num_of_leave"
                  type="text"
                  class="appearance-none border w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none"
                  ...
                  @keyup="formValidation('num_of_leave')"
                />
                <div class="text-red-500 text-xs mt-1">
                  {{ nameErrorMessage }}
                </div>
  ...
  function openModalEdit(dataInfo) {
    ...
    leaveType.value = dataInfo.leaveType;
    num_of_leave.value = dataInfo.num_of_leave;
  ...
  const fetchDataList = async () => {
    try {
      ...
      const response = await axios.get("/staff/v1/erp/leaveType/get/list", config);
  ...
  function validateName() {
    if (!leaveType.value || leaveType.value.length === 0) {
    ...
    if (formValidation("submit")) {
      try {
        ...
        const payload = {
          ...
          num_of_leave: num_of_leave.value,
         
          dataId: dataId.value || null,
        };
        const response = await axios.post(
          "/staff/v1/erp/leaveType/insert/update",
          payload,
          config
        );
  ...
  async function deleteInfo() {
    try {
      ...
      const response = await axios.post("/staff/v1/erp/leaveType/delete", payload, config);
      if (response.data.msgFlag) {
        toast.success(response.data.msg);
        await fetchDataList();
        closeModal();
      ...
      toast.error("An error occurred while deleting the leaveType.");
    }
  }
