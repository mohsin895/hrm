<template>
    <div class="loyel-erp-award mt-10 ml-10 mr-10 bg-white p-6">
      <div class="card w-full">
        <div class="p-2 mb-4">
          <div
            class="bg-customCyan hover:bg-hoverCyan p-2 w-52 cursor-pointer"
            @click="openModal"
          >
            ...
            <div class="pl-6 pr-6 modal-body">
              ...
              <div class="mb-4">
                <label
                  ...
                  for="giftName"
                >
                  <span :class="{ 'text-red-500': nameError }">Description</span>
                </label>
                <input
                  v-model="description"
                  type="text"
        ...
        <div class="bg-white shadow-lg w-full max-w-2xl" @click.stop>
          ...
          <form @submit.prevent="deleteInfo">
            <div class="modal-body p-6">
              <span> Are you sure you want to delete this noticeboard: </span>
            </div>
  ...
  const fetchDataList = async () => {
    try {
      ...
      const response = await axios.get("/staff/v1/erp/noticeboard/get/list", config);
  ...
  async function deleteInfo() {
    try {
      ...
      const response = await axios.post("/staff/v1/erp/noticeboard/delete", payload, config);
      if (response.data.msgFlag) {
        toast.success(response.data.msg);
        await fetchDataList();
        closeModal();
      ...
      console.error("Error deleting noticeboard:", error);
      toast.error("An error occurred while deleting the noticeboard.");
    }
  }
