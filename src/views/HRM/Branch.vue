<template>
        ...
        <div class="overflow-x-auto">
          ...
          <thead>
            <tr>
              ...
              <th class="border border-slate-300">Branch Name</th>
              <th class="border border-slate-300">Branch Address</th>
        ...
        <div class="bg-white shadow-lg w-full max-w-lg " @click.stop>
          ...
          <form @submit.prevent="submitForm">
            <div class="pl-6 pr-6 modal-body">
              <div class="mb-4">
                <label class="block text-gray-700 text-sm font-bold mb-2" for="branchName">
                  <span :class="{ 'text-red-500': nameError }">Brnach</span>
                </label>
                <input
                  v-model="branchName"
                  type="text"
                  ...
                  @keyup="formValidation('branchName')"
                />
              ...
              <div class="mb-4">
                <label class="block text-gray-700 text-sm font-bold mb-2" for="address">
                  <span :class="{ 'text-red-500': addressError }">Branch Address</span>
                </label>
                <textarea id="local-address" rows="3" v-model="address" class="py-2 px-3 block w-full border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6"></textarea>
          ...
          <form @submit.prevent="deleteInfo">
            <div class="modal-body p-6">
              ...
              <div class="bg-deleteColor p-4 border-l-8 border-borderColor">
                ...
                <span class="font-bold">employees</span> associated with this Branch will also be deleted.
              </div>
            </div>
  ...
  function openModalEdit(dataInfo) {
    ...
    branchName.value = dataInfo.branchName;
    address.value = dataInfo.address;
    isModalOpen.value = true;
  }
  ...
  const fetchDataList = async () => {
    try {
      ...
      const response = await axios.get('/staff/v1/erp/branch/get/list', config);
  ...
  function validateName() {
    if (!branchName.value || branchName.value.length === 0) {
      nameError.value = true;
      nameErrorMessage.value = 'Branch Name Field cannot be empty';
      return false;
    } else {
      nameError.value = false;
      nameErrorMessage.value = '';
  ...
  async function submitForm() {
    ...
    if (formValidation('submit') ) {
      ...
      try {
        ...
        const response = await axios.post('/staff/v1/erp/branch/insert/update', payload, config);
  ...
  async function deleteInfo() {
    try {
      ...
      const response = await axios.post('/staff/v1/erp/branch/delete', payload, config);
      if (response.data.msgFlag) {
        toast.success(response.data.msg);
        await fetchDataList();
        closeModal();
